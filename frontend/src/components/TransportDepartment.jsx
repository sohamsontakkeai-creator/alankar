  ...
  const handleApprovalDecision = async () => {
    try {
      ...
      const payload = {
        ...
        reviewedBy: user?.fullName || user?.username || 'Unknown User',
        approvedBy: user?.fullName || user?.username || 'Unknown User',
        rejectedBy: user?.fullName || user?.username || 'Unknown User'
      };
  ...
  const handleAssignVehicle = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Vehicle assigned - status is now 'sent_to_dispatch', waiting for dispatch to load
        toast({ title: "Success", description: "Vehicle assigned and sent to dispatch for loading" });
