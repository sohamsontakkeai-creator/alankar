...
const WatchmanDepartment = () => {
    ...
    // Auto-refresh every 5 seconds (high frequency for watchman)
    ...
    const handleVerifyPickup = async (action) => {
        try {
            // Mandatory photo validation for send_in and release actions
            if (action === 'send_in' || action === 'release') {
                const hasPhoto = action === 'send_in' 
                    ? (capturedSendInPhoto || manualSendInFile)
                    : (capturedReleasePhoto || manualReleaseFile);

                if (!hasPhoto) {
                    toast({
                        title: "Photo Required",
                        description: `A photo is mandatory for ${action === 'send_in' ? 'Verify & Send In' : 'Verify & Release'} action. Please capture or upload a photo before proceeding.`,
                        variant: "destructive"
                    });
                    return;
                }
            ...
            if ((action === 'send_in' && capturedSendInPhoto && capturedSendInPhoto !== 'manual') ||
                ...
                formData.append('verifiedBy', user?.fullName || user?.username || 'Unknown User');
                ...
                // This should not happen for send_in/release actions due to validation above
                const requestData = {
                    ...verificationForm,
                    action: action,
                    verifiedBy: user?.fullName || user?.username || 'Unknown User'
                };
                                        ...
                                        <TableBody>
                                            {pendingPickups.map((pickup) => (
                                                <TableRow key={pickup.gatePassId}>
                                                    ...
                                                    <TableCell className="font-medium text-gray-800">{formatToIST(pickup.issuedAt)}</TableCell>
                                                    <TableCell>
                                            ...
                                            {filteredAllGatePasses.map((gatePass) => {
                                                ...
                                                return (
                                                    <TableRow key={gatePass.gatePassId}>
                                                        ...
                                                        <TableCell className="text-gray-800">{formatToIST(gatePass.issuedAt)}</TableCell>
                                                        <TableCell className="text-gray-800">
                                                            {formatToIST(gatePass.verifiedAt)}
                                                        </TableCell>
                                                    </TableRow>
                                                );
                                            })}
                                            ...
                                            {companyVehicleReturns.length === 0 ? (
                                                ...
                                                companyVehicleReturns.map((item) => (
                                                    <TableRow key={item.id || item.vehicleId || item.data?.vehicleId}>
                                                        ...
                                                        <TableCell className="text-gray-800">{formatToIST(item.timestamp)}</TableCell>
                                                        <TableCell>
                                                            <div className="flex gap-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mt-3">
                                    ...
                                    <p className="text-xs text-orange-700 mt-1">
                                        Taking a photo is mandatory for {selectedGatePass?.dispatchStatus === 'loaded' ? 'Verify & Release' : 'Verify & Send In'} action. 
                                        Please capture or upload a photo before proceeding.
                                    </p>
                                </div>
                        ...
                        {(selectedGatePass?.dispatchStatus === 'ready_for_pickup' || 
                          selectedGatePass?.dispatchStatus === 'ready_for_load') ? (
                            <Button
                                ...
                                className="bg-blue-600 hover:bg-blue-700"
                                disabled={
                                    selectedGatePass?.status === 'entered_for_pickup' || 
                                    selectedGatePass?.status === 'loaded' || 
                                    selectedGatePass?.status === 'verified' ||
                                    (!capturedSendInPhoto && !manualSendInFile)
                                }
                                title={(!capturedSendInPhoto && !manualSendInFile) ? "Photo is required for verification" : ""}
                            >
                                <CheckCircle className="h-4 w-4 mr-1" />
                                Verify & Send In
                                {(!capturedSendInPhoto && !manualSendInFile) && (
                            ...
                            <Button 
                                ...
                                className="bg-green-600 hover:bg-green-700"
                                disabled={(!capturedReleasePhoto && !manualReleaseFile)}
                                title={(!capturedReleasePhoto && !manualReleaseFile) ? "Photo is required for verification" : ""}
                            >
                                <CheckCircle className="h-4 w-4 mr-1" />
                                Verify & Release
                                {(!capturedReleasePhoto && !manualReleaseFile) && (
