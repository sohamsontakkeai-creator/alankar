...
const DispatchDepartment = () => {
    ...
    // Auto-refresh every 5 seconds (high frequency for dispatch)
    ...
    useEffect(() => {
        if (dispatchSummary) {
            // Calculate total pending orders from all four sections in the "Pending Orders" tab:
            ...
            // 4. Company Vehicle Loading Queue (delivery_type = 'transport' AND status = 'ready_for_load')
            const totalPendingCount = (dispatchSummary.pendingOrders || 0) + 
                                     ...
                                     (dispatchSummary.companyVehicleLoadingQueue || 0);
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        <OrderStatusBar className="mb-4" />
             ...
             <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                <TabsList className={`flex md:grid md:grid-cols-${user?.designation === 'Manager' ? '5' : '4'} w-full overflow-x-auto overflow-y-hidden md:overflow-x-hidden whitespace-nowrap bg-gray-100 border border-gray-300 pl-24 md:pl-0`}>
                    ...
                    {user?.designation === 'Manager' && (
                        <TabsTrigger value="leaveApprovals" className="text-gray-800">
                ...
                <TabsContent value="pending" className="space-y-4">
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {dispatchOrders.filter(o => {
                                        const deliveryType = o.deliveryType?.toLowerCase().trim();
                                        const originalDeliveryType = o.originalDeliveryType?.toLowerCase().trim();
                                        const isSelfDelivery = deliveryType === 'self' || originalDeliveryType === 'self delivery';
                                        return isSelfDelivery && ['ready_for_load', 'entered_for_pickup'].includes(o.status);
                                    }).map(order => (
                                        <TableRow key={order.id}>
                                            <TableCell className="font-medium text-gray-800">{order.orderNumber}</TableCell>
                                            ...
                                            <TableCell>
                                                <div className="flex gap-2">
                                                    <Button
                                                        ...
                                                        onClick={async () => {
                                                            const res = await fetch(`${API_BASE}/dispatch/self/loaded/${order.id}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ notes: 'Loaded by dispatch', loadedBy: user?.fullName || user?.username || 'Unknown User' }) });
                                                            if (res.ok) {
                                ...
                                <TableBody>
                                    {dispatchOrders
                                        .filter(o => {
                                            const deliveryType = o.deliveryType?.toLowerCase().trim();
                                            const originalDeliveryType = o.originalDeliveryType?.toLowerCase().trim();
                                            const isPartLoadDeliveryType = deliveryType === 'part load' || deliveryType === 'partload';
                                            const isPartLoadOriginalType = originalDeliveryType === 'part load' || originalDeliveryType === 'partload';
                                            const isPartLoad = isPartLoadDeliveryType || isPartLoadOriginalType;
                                            const hasCorrectStatus = ['ready_for_load', 'ready_for_pickup', 'entered_for_pickup'].includes(o.status);
                                            if ((deliveryType && deliveryType.includes('part')) || (originalDeliveryType && originalDeliveryType.includes('part'))) {
                                                console.log('Part Load Debug:', {
                                                    orderNumber: o.orderNumber,
                                                    deliveryType: o.deliveryType,
                                                    originalDeliveryType: o.originalDeliveryType,
                                                    deliveryTypeLower: deliveryType,
                                                    originalDeliveryTypeLower: originalDeliveryType,
                                                    status: o.status,
                                                    isPartLoad,
                                                    hasCorrectStatus,
                                                    willShow: isPartLoad && hasCorrectStatus
                                                });
                                            }
                                            return isPartLoad && hasCorrectStatus;
                                        })
                                        .map(order => (
                                            <TableRow key={order.id}>
                                                <TableCell className="font-medium text-gray-800">{order.orderNumber}</TableCell>
                                                        ...
                                                        <Button
                                                            ...
                                                            onClick={async () => {
                                                                const res = await fetch(`${API_BASE}/dispatch/part-load/loaded/${order.id}`, { 
                                                                    ...
                                                                    body: JSON.stringify({ notes: 'Part Load loaded by dispatch', loadedBy: user?.fullName || user?.username || 'Unknown User' }) 
                                                                });
                                                                if (res.ok) {
                    ...
                    {/* Company Vehicle Loading Queue */}
                    <Card className="bg-white border border-gray-300 shadow-sm">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2 text-gray-800">
                                <Truck className="h-5 w-5" /> Company Vehicle Loading Queue
                            </CardTitle>
                            <CardDescription className="text-gray-600">
                                Company/Free delivery orders with vehicles assigned by transport, ready for loading.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="overflow-x-auto">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead className="text-black">Product</TableHead>
                                        <TableHead className="text-black">Vehicle</TableHead>
                                        <TableHead className="text-black">Transporter</TableHead>
                                        <TableHead className="text-black">Status</TableHead>
                                        <TableHead className="text-black">Actions</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {dispatchOrders
                                        .filter(o => {
                                            const deliveryType = o.deliveryType?.toLowerCase().trim();
                                            const originalDeliveryType = o.originalDeliveryType?.toLowerCase().trim();
                                            const isTransport = ['transport', 'company', 'free'].includes(deliveryType || '');
                                            const isCompanyOrFree = ['company delivery', 'free delivery', 'company', 'free'].includes(originalDeliveryType || '') || ['company', 'free'].includes(deliveryType || '');
                                            return isTransport && isCompanyOrFree && o.status === 'ready_for_load';
                                        })
                                        .map(order => (
                                            <TableRow key={order.id}>
                                                <TableCell className="font-medium text-gray-800">{order.orderNumber}</TableCell>
                                                ...
                                                <TableCell>
                                                    <div className="flex gap-2">
                                                        <Button
                                                            ...
                                                            variant="default"
                                                            onClick={async () => {
                                                                const res = await fetch(`${API_BASE}/dispatch/company/loaded/${order.id}`, { 
                                                                    method: 'POST', 
                                                                    headers: { 'Content-Type': 'application/json' }, 
                                                                    body: JSON.stringify({ notes: 'Company vehicle loaded by dispatch', loadedBy: user?.fullName || user?.username || 'Unknown User' }) 
                                                                });
                                                                if (res.ok) {
                                                                    toast({ title: 'Loaded', description: `Company vehicle for order ${order.orderNumber} marked loaded and ready for delivery` });
                                                                    fetchData();
                                                                } else {
                                                                    const err = await res.json().catch(() => ({}));
                                                                    toast({ title: 'Failed', description: err.error || 'Try again', variant: 'destructive' });
                                    ...
                                    {dispatchOrders.filter(o => {
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={7} className="text-center py-8 text-gray-600">
                                                No company vehicles ready for loading.
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
