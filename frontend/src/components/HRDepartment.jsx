...
import {
  Users, Calendar, IndianRupee, UserPlus, TrendingUp, Award,
  Plus, PlusCircle, Edit, Trash2, X, Search, Filter, Download,
...
const HRDepartment = () => {
  ...
  const [tourIntimations, setTourIntimations] = useState([]);
  ...
  const [leaveStatusDate, setLeaveStatusDate] = useState(new Date().toISOString().split('T')[0]);
  const { toast } = useToast();
  
  // Stable handler for search input to prevent re-render issues
  const handleLeaveSearchChange = useCallback((e) => {
  ...
  const openModal = (type, employee = null) => {
    ...
    // Fetch employees on leave when opening the leave status modal
    if (type === 'viewLeaveStatus') {
    ...
    if ((type === 'editEmployee' || type === 'viewEmployee') && employee) {
      ...
      setModalFormData({
        ...
        phone: '',
        designation: '',
        customDesignation: '',  // Reset custom designation
        department: '',
        joiningDate: '',
        address: '',
        salary: '',
  ...
  const handleSubmit = async () => {
    switch (modalType) {
      ...
      case 'editEmployee':
        ...
        const finalDesignation = modalFormData.designation === 'Other' 
          ? modalFormData.customDesignation 
          : modalFormData.designation;
        ...
        if (modalFormData.designation === 'Other' && !modalFormData.customDesignation) {
          toast({ title: "Missing Information", description: "Please enter a custom designation.", variant: "destructive" });
          return;
        }
        ...
        try {
          const payload = {
            ...
            designation: finalDesignation,  // Use final designation (custom if "Other" selected)
  ...
  // Tour Intimation Functions
  const handleSubmitTour = async () => {
    try {
      if (!modalFormData.employeeId || !modalFormData.tourPurpose || !modalFormData.destination || 
          !modalFormData.startDate || !modalFormData.endDate) {
        toast({ title: "Missing Information", description: "Please fill all required fields.", variant: "destructive" });
        return;
      }

      const url = modalType === 'createTour' 
        ? `${API_BASE}/hr/tours`
        : `${API_BASE}/hr/tours/${modalFormData.id}`;
      
      const method = modalType === 'createTour' ? 'POST' : 'PUT';
      ...
      toast({ 
        title: 'Success', 
        description: modalType === 'createTour' ? 'Tour request created successfully' : 'Tour request updated successfully'
      });
      
      closeModal();
      await fetchTourIntimations();
    } catch (error) {
  ...
  const fetchData = async () => {
    ...
    try {
      switch (activeTab) {
        ...
        case 'tours':
          await fetchTourIntimations();
          await fetchEmployees(); // For employee dropdown
          break;
        case 'recruitment':
  ...
  // Auto-refresh data every 10 seconds (pauses when user is typing)
  const { isRefreshing: autoRefreshing, lastRefreshTime, isPaused } = useAutoRefresh(fetchData, 10000, {
    enabled: true,
    pauseOnInput: true
  });
  ...
  const fetchTourIntimations = async () => {
    const response = await fetch(`${API_BASE}/hr/tours`);
    if (!response.ok) throw new Error('Failed to fetch tour intimations');
    const data = await response.json();
    setTourIntimations(data);
  };

  const fetchEmployeesOnLeave = async (date = null) => {
    try {
      const targetDate = date || leaveStatusDate;
      const response = await fetch(`${API_BASE}/hr/employees/on-leave?date=${targetDate}`);
      if (response.ok) {
        const data = await response.json();
        setEmployeesOnLeave(data.employees || []);
      } else {
        toast({
          title: "Error",
          description: "Failed to fetch employees on leave",
          variant: "destructive"
        });
      }
    } catch (error) {
...
const DashboardView = ({ employees = [] }) => {  // ✅ Accept employees as prop (default to empty array)
  ...
  const AttendanceView = () => {
    ...
    const rejectedLeaves = leaveRequests.filter(l => l.status === 'rejected').length;
      ...
      }, [leaveRequests, leaveSearchQuery]);
  ...
  const TourIntimationView = () => {
    ...
    const filteredTours = tourIntimations.filter(tour => {
      const matchesSearch = tour.employeeName.toLowerCase().includes(tourSearchQuery.toLowerCase()) ||
                           tour.destination.toLowerCase().includes(tourSearchQuery.toLowerCase()) ||
                           tour.tourPurpose.toLowerCase().includes(tourSearchQuery.toLowerCase());
      const matchesStatus = !tourStatusFilter || tour.status === tourStatusFilter;
      return matchesSearch && matchesStatus;
    });

    const handleCreateTour = () => {
      setModalType('createTour');
      setModalFormData({});
      setShowModal(true);
    };

    const handleEditTour = (tour) => {
      setModalType('editTour');
      setModalFormData(tour);
      setShowModal(true);
    };

    const handleApproveTour = async (tourId) => {
      try {
        const response = await fetch(`${API_BASE}/hr/tours/${tourId}/approve`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ approverId: user.id })
        });

        if (!response.ok) throw new Error('Failed to approve tour');
    ...
    const handleRejectTour = async (tourId) => {
      ...
      try {
        const response = await fetch(`${API_BASE}/hr/tours/${tourId}/reject`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ approverId: user.id, reason })
        });

        if (!response.ok) throw new Error('Failed to reject tour');

        toast({ title: 'Success', description: 'Tour rejected' });
        await fetchTourIntimations();
      } catch (error) {
        toast({ title: 'Error', description: error.message, variant: 'destructive' });
      }
    ...
    const getStatusBadge = (status) => {
      ...
      const colors = {
        'pending': 'bg-yellow-400 text-gray-900 border-2 border-yellow-500 font-semibold',
        'management_approved': 'bg-blue-400 text-white border-2 border-blue-500 font-semibold',
        'approved': 'bg-green-500 text-white border-2 border-green-600 font-semibold',
        'rejected': 'bg-red-500 text-white border-2 border-red-600 font-semibold',
        'management_rejected': 'bg-red-400 text-white border-2 border-red-500 font-semibold',
        'completed': 'bg-blue-500 text-white border-2 border-blue-600 font-semibold',
        'cancelled': 'bg-gray-400 text-white border-2 border-gray-500 font-semibold'
      };
      return colors[statusLower] || 'bg-gray-400 text-white border-2 border-gray-500 font-semibold';
    };

    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-900">Tour Intimations</h2>
        ...
        <div className="flex gap-4">
          ...
          >
            ...
            <option value="pending">Pending (Awaiting Management)</option>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredTours.length > 0 ? filteredTours.map((tour) => (
                <tr key={tour.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{tour.employeeName}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{tour.tourPurpose}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{tour.destination}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                    {new Date(tour.startDate).toLocaleDateString()} - {new Date(tour.endDate).toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{tour.durationDays} days</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">₹{tour.estimatedCost?.toLocaleString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Badge className={getStatusBadge(tour.status)}>{tour.status}</Badge>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => handleEditTour(tour)}>
                        <Eye className="w-4 h-4" />
                      </Button>
                      {(tour.status === 'management_approved' || tour.status === 'MANAGEMENT_APPROVED') && (
                        <>
                          <Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => handleApproveTour(tour.id)}>
                            <CheckCircle className="w-4 h-4" />
                          </Button>
                          <Button size="sm" variant="destructive" onClick={() => handleRejectTour(tour.id)}>
                            <XCircle className="w-4 h-4" />
                          </Button>
                        </>
                      )}
                      {(tour.status === 'pending' || tour.status === 'PENDING') && (
                        <span className="text-xs text-gray-500 italic">Waiting for management approval</span>
                      )}
                    </div>
                  </td>
                </tr>
  ...
  return (
    <div className="min-h-screen bg-gray-100">
          ...
          <div className="flex flex-col space-y-4 sm:flex-row sm:justify-between sm:items-center sm:space-y-0">
            ...
            <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Human Resource System</h1>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <Button
                onClick={() => {
                  setModalType('viewLeaveStatus');
                  setShowModal(true);
                }}
                variant="outline"
                className="bg-orange-50 hover:bg-orange-100 text-orange-700 border-orange-300 flex items-center gap-2 w-full sm:w-auto"
              >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="overflow-x-auto">
        <TabsList className="flex md:grid md:grid-cols-6 w-full overflow-x-auto overflow-y-hidden md:overflow-x-hidden whitespace-nowrap bg-gray-100 border border-gray-300 pl-0">
          ...
          <TabsTrigger value="tours" className="text-gray-800">Tour Intimation</TabsTrigger>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col bg-white border-gray-200 text-gray-900">
            <DialogHeader className="flex-shrink-0">
              <DialogTitle className="text-gray-900">
                ...
                {modalType === 'createTour' && 'New Tour Request'}
                {modalType === 'editTour' && 'Tour Details'}
                ...
                {modalType === 'viewLeaveStatus' && 'Employees on Leave'}
              </DialogTitle>
            </DialogHeader>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => {
                        setModalFormData({
                          ...modalFormData, 
                          designation: value,
                          customDesignation: value === 'Other' ? modalFormData.customDesignation : ''
                        });
                      }}
                    >
                      <SelectTrigger className="bg-white text-gray-900 border-gray-300">
                    ...
                    {modalFormData.designation === 'Other' && (
                      <div className="mt-2">
                        <Input
                          id="customDesignation"
                          placeholder="Enter custom designation"
                          value={modalFormData.customDesignation || ''}
                          className="bg-white text-gray-900 border-gray-300"
                          onChange={(e) => setModalFormData({...modalFormData, customDesignation: e.target.value})}
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <Select value={modalFormData.managerId || ''} onValueChange={(value) => setModalFormData({...modalFormData, managerId: value === '' ? null : value})}>
                      ...
                      <SelectContent>
                        ...
                        {employees
                          .filter((emp) => {
                            // Only show employees with Manager designation
                          ...
                          .map((emp) => (
                            <SelectItem key={emp.id} value={emp.id.toString()}>
                              {emp.fullName || emp.name || `${emp.firstName ?? ''} ${emp.lastName ?? ''}`.trim()} ({emp.designation})
                            </SelectItem>
                          ))}
                      </SelectContent>
                    </Select>
    ...
    <div>
      ...
      />
           ...
           {/* View Job Posting Modal */}
            ...
            {/* Tour Intimation Modals */}
            {(modalType === 'createTour' || modalType === 'editTour') && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="employeeId" className="text-gray-900">Employee *</Label>
                    <Select
                      value={modalFormData.employeeId || ''}
                      onValueChange={(value) => {
                  ...
                  <div>
                    <Label htmlFor="destination" className="text-gray-900">Destination *</Label>
                    <Input
                      id="destination"
                ...
                <div>
                  <Label htmlFor="tourPurpose" className="text-gray-900">Tour Purpose *</Label>
                  <Textarea
                    id="tourPurpose"
                    value={modalFormData.tourPurpose || ''}
                    onChange={(e) => setModalFormData({...modalFormData, tourPurpose: e.target.value})}
                    placeholder="Describe the purpose of the tour"
                    rows={3}
                    className="bg-white text-black border-gray-300"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="estimatedCost" className="text-gray-900">Estimated Cost (₹)</Label>
                    <Input
                      id="estimatedCost"
                      type="number"
                      value={modalFormData.estimatedCost || ''}
                      onChange={(e) => setModalFormData({...modalFormData, estimatedCost: parseFloat(e.target.value)})}
                      placeholder="0"
                      className="bg-white text-black border-gray-300"
                    />
                  </div>
                  <div>
                    <Label htmlFor="advanceRequired" className="text-gray-900">Advance Required (₹)</Label>
                    <Input
                      id="advanceRequired"
                      type="number"
                      value={modalFormData.advanceRequired || ''}
                      onChange={(e) => setModalFormData({...modalFormData, advanceRequired: parseFloat(e.target.value)})}
                      placeholder="0"
                      className="bg-white text-black border-gray-300"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="travelMode" className="text-gray-900">Travel Mode</Label>
                    <Select
                      value={modalFormData.travelMode || ''}
                      onValueChange={(value) => setModalFormData({...modalFormData, travelMode: value})}
                    >
                      <SelectTrigger className="bg-white text-black border-gray-300">
                  ...
                  <div className="flex items-center space-x-2 pt-8">
                    <input
                      ...
                      checked={modalFormData.accommodationRequired || false}
                      onChange={(e) => setModalFormData({...modalFormData, accommodationRequired: e.target.checked})}
                      className="rounded"
                    />
                    <Label htmlFor="accommodationRequired" className="text-gray-900">Accommodation Required</Label>
                  </div>
                </div>

                <div>
                  <Label htmlFor="remarks" className="text-gray-900">Remarks</Label>
                  <Textarea
                    id="remarks"
                ...
                {modalType === 'editTour' && modalFormData.status && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label className="text-sm text-gray-600">Status</Label>
                        <Badge className={`mt-1 ${modalFormData.status === 'APPROVED' ? 'bg-green-100 text-green-800' : modalFormData.status === 'REJECTED' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
                          {modalFormData.status}
                        </Badge>
                      </div>
                      {modalFormData.approverName && (
                        <div>
                          <Label className="text-sm text-gray-600">Approved/Rejected By</Label>
                          <p className="text-sm font-medium mt-1">{modalFormData.approverName}</p>
                        </div>
                      )}
                    </div>
                    {modalFormData.rejectionReason && (
                      <div className="mt-2">
                        <Label className="text-sm text-gray-600">Rejection Reason</Label>
                        <p className="text-sm mt-1">{modalFormData.rejectionReason}</p>
                      </div>
                    )}
                  </div>
                )}
          ...
          {/* View Application Modal */}
            ...
            {/* View Leave Status Modal */}
            {modalType === 'viewLeaveStatus' && (
              <div className="space-y-4">
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-lg font-semibold text-orange-900">Employees on Leave</h4>
                    <div className="flex items-center gap-2">
                      <Label htmlFor="leaveDate" className="text-sm font-medium">Date:</Label>
                      <Input
                        id="leaveDate"
                        type="date"
                        value={leaveStatusDate}
                        onChange={(e) => {
                          setLeaveStatusDate(e.target.value);
                          fetchEmployeesOnLeave(e.target.value);
                        }}
                        className="w-40 bg-white border-orange-300"
                      />
                    </div>
                  </div>
                  ...
                  {employeesOnLeave.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-lg font-medium">No employees on leave</p>
                      <p className="text-sm">All employees are available on {new Date(leaveStatusDate).toLocaleDateString()}</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <p className="text-sm text-orange-700 mb-3">
                        <strong>{employeesOnLeave.length}</strong> employee{employeesOnLeave.length !== 1 ? 's' : ''} on leave on {new Date(leaveStatusDate).toLocaleDateString()}
                      </p>
                      
                      <div className="max-h-96 overflow-y-auto">
                        {employeesOnLeave.map((employee, index) => (
                          <div key={employee.id} className="bg-white border border-orange-200 rounded-lg p-4 shadow-sm">
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-2">
                                  <h5 className="font-semibold text-gray-900">{employee.fullName}</h5>
                                  <Badge variant="outline" className="text-green-xs">
                                    {employee.leaveType}
                                  </Badge>
                                </div>
                                <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                                  ...
                                  <div>
                                    <span className="font-medium">Days:</span> {employee.daysRequested}
                                  </div>
                                </div>
                                <div className="mt-2 text-sm">
                                  <div className="text-gray-600">
                                    <span className="font-medium">Period:</span> {new Date(employee.startDate).toLocaleDateString()} - {new Date(employee.endDate).toLocaleDateString()}
                                  </div>
                                  {employee.reason && (
...
{/* Edit Payroll Modal */}
            ...
            <DialogFooter className="flex-shrink-0 border-t border-gray-200 pt-4 mt-2 bg-gray-50">
              ...
              {(modalType === 'createTour' || modalType === 'editTour') && (
                <Button onClick={handleSubmitTour}>
                  {modalType === 'createTour' ? 'Submit Request' : 'Update Request'}
                </Button>
              )}
              {modalType !== 'viewEmployee' && modalType !== 'viewPayroll' && modalType !== 'generatePayroll' && modalType !== 'markAttendance' && modalType !== 'editPayroll' && modalType !== 'viewLeaveStatus' && modalType !== 'createTour' && modalType !== 'editTour' && (
