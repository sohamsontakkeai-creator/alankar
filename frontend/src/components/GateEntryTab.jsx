    // Multi-frame capture for registration (improved version with retry logic)
    const captureMultiplePhotos = async (numPhotos = 7, intervalMs = 400) => {
        ...
        for (let i = 0; i < numPhotos; i++) {
            ...
            while (!photo && attempts < maxRetries) {
                ...
                if (videoRef.current && videoRef.current.paused) {
                    try {
                        ...
                        console.warn('⚠️ Could not resume video');
                    }
                }
            ...
            if (photo) {
                photos.push(photo);
                console.log(`✅ Photo ${i + 1}/${numPhotos} captured`);
            }
...
const GateEntryTab = () => {
    ...
    const capturePhoto = (shouldSetState = false) => {
        if (!videoRef.current || !canvasRef.current) {
            console.warn('❌ capturePhoto: Video or canvas ref not available');
            return null;
        }
        ...
        // Accept readyState 2, 3, or 4 (not just 4)
        // 2 = HAVE_CURRENT_DATA, 3 = HAVE_FUTURE_DATA, 4 = HAVE_ENOUGH_DATA
        if (video.readyState < 2) {
            console.warn(`❌ capturePhoto: Video not ready. readyState=${video.readyState} (need >= 2)`);
            return null;
        }
        
        // Check if video has valid dimensions
        if (!video.videoWidth || !video.videoHeight) {
            console.warn('❌ capturePhoto: Video dimensions invalid', {
        ...
        try {
            ...
            console.log(`✅ capturePhoto: Successfully captured photo (${canvas.width}x${canvas.height})`);
            
            // Only update state if explicitly requested (during multi-photo capture, don't update to avoid re-renders)
            if (shouldSetState) {
            ...
            console.error('❌ capturePhoto: Error during capture:', err);
            return null;
        }
    };
    ...
    // Capture multiple photos and auto-register with retry logic
    const handleCaptureLivePhoto = async () => {
        ...
        if (!userForm.name.trim() || !userForm.phone.trim()) {
            toast({
                title: "Error",
                description: "Please fill in name and phone first",
                variant: "destructive"
            });
            return;
        }
        ...
        try {
            setRecognitionStatus('🎥 Preparing camera for photo capture...');
            let photos = [];
            const requiredPhotos = 7;
            const maxRetries = 3; // Try up to 3 times per photo
            
            // Wait for video to stabilize before capturing
            await new Promise(res => setTimeout(res, 500));
            
            setRecognitionStatus('🎥 Capturing 7 photos for face encoding...');
            console.log('🎬 Starting photo capture sequence...');
            
            // Capture required number of photos with retry logic
            for (let i = 0; i < requiredPhotos; i++) {
                let photo = null;
                let attempts = 0;
                
                // Retry logic: if photo fails, try again
                while (!photo && attempts < maxRetries) {
                    attempts++;
                    
                    // Ensure video is still playing before capture
                    if (videoRef.current && videoRef.current.paused) {
                        console.warn('⚠️ Video was paused, attempting to resume...');
                        try {
                            videoRef.current.play();
                        } catch (e) {
                            console.warn('⚠️ Could not resume video:', e);
                        }
                    }
                    ...
                    if (!photo && attempts < maxRetries) {
                        ...
                        const retryDelay = 150 + (attempts * 50); // 150ms, 200ms, 250ms
                        console.warn(`⚠️ Capture attempt ${attempts}/${maxRetries} failed, retrying in ${retryDelay}ms...`);
                ...
                if (photo) {
                    photos.push(photo);
                    console.log(`✅ Photo ${i + 1}/7 captured successfully`);
                    setRecognitionStatus(`✅ Captured ${i + 1}/${requiredPhotos} photos...`);
                } else {
                    console.warn(`⚠️ Photo ${i + 1}/7 failed after ${maxRetries} attempts`);
                    setRecognitionStatus(`⚠️ Capture ${i + 1}/${requiredPhotos} failed, continuing...`);
                }
                
                // Wait 400ms before next capture
                await new Promise(res => setTimeout(res, 400));
            }

            console.log(`📸 Capture complete. Got ${photos.length}/${requiredPhotos} photos`);

            if (photos.length < 3) {
                toast({
                    title: "❌ Insufficient Photos",
                    description: `Only captured ${photos.length} photos. Need at least 3. Please ensure camera is working and lighting is good.`,
                    variant: "destructive"
                });
                setRecognitionStatus('System Ready');
                return;
            ...
            // Auto-register with all captured photos
            setRecognitionStatus(`🚀 Registering user with ${photos.length} face photos...`);
            console.log(`📤 Sending ${photos.length} photos to backend for registration...`);
            ...
            const response = await fetch(`${API_BASE}/gate-entry/register`, {
                ...
                body: JSON.stringify({
                    name: userForm.name,
                    phone: userForm.phone,
                    photos: photos // IMPORTANT: Send array of photos!
                }),
            });
            ...
            if (response.ok) {
                const encodingCount = result.face_encodings_count || photos.length;
                console.log(`✅ Registration successful! Generated ${encodingCount} face encodings`);
                toast({
                    title: "✅ User Registered Successfully",
                    description: `Registered with ${encodingCount} face encodings from ${photos.length} photos!`,
                });
                ...
                toast({
                    ...
                    description: result.message || "Failed to register user with captured photos",
                    variant: "destructive"
                });
                setRecognitionStatus('System Ready');
            }
        } catch (error) {
            console.error('❌ Error during capture/registration:', error);
            toast({
                title: "Error",
