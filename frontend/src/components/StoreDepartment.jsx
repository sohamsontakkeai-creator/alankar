...
const StoreDepartment = () => {
  ...
  const fetchAllData = async () => {
    await Promise.all([fetchInventory(), fetchPurchaseOrders()]);
  };

  const { isRefreshing: autoRefreshing, lastRefreshTime, isPaused } = useAutoRefresh(
    fetchAllData, 
    8000,
    { enabled: true, pauseOnInput: true }
  );
  ...
  const handleStoreCheck = async (orderId) => {
    try {
      const res = await fetch(`${API_URL}/store/orders/${orderId}/check-stock`, {
        ...
        body: JSON.stringify({ checkedBy: user?.fullName || user?.username || 'Unknown User' }),
      });
      const data = await res.json();
    ...
    try {
      const res = await fetch(
        `${API_URL}/store/orders/${orderId}/verify-purchase`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ verifiedBy: user?.fullName || user?.username || 'Unknown User' }),
        }
      );
      const data = await res.json();
  ...
  const handleAddItem = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/store/inventory`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...newItem, addedBy: user?.fullName || user?.username || 'Unknown User' }),
      });
      const data = await res.json();
                      ...
                      >
                        ...
                        {order.materials && order.materials.length > 0 && (
                          <div className="mb-2 bg-blue-50 border border-blue-200 rounded p-2">
                            <span className="text-blue-900 font-semibold text-sm">Required Materials (Shortage):</span>
                        ...
                        {order.extraMaterials && order.extraMaterials.length > 0 && (
                          <div className="mb-2 bg-green-50 border border-green-200 rounded p-2">
                            <span className="text-green-900 font-semibold text-sm">Extra Materials (Additional):</span>
                            <ul className="list-disc list-inside ml-4 mt-1">
                              {order.extraMaterials.map((mat, idx) => (
                                <li key={idx} className="text-gray-700 text-sm">
                                  {typeof mat === 'string' ? mat : `${mat.name || mat} (Qty: ${mat.quantity || '-'}, Unit Cost: ₹${mat.unit_cost || 0})`}
                                </li>
                              ))}
