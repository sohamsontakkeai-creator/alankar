...
class LeaveStatus(Enum):
    PENDING = "PENDING"  # Waiting for manager approval (if manager exists) or HR approval
    MANAGER_APPROVED = "MANAGER_APPROVED"  # Manager approved, waiting for HR
    APPROVED = "APPROVED"  # Final approval by HR
...
class TourStatus(Enum):
    PENDING = "PENDING"  # Waiting for management approval
    MANAGEMENT_APPROVED = "MANAGEMENT_APPROVED"  # Management approved, waiting for HR
    APPROVED = "APPROVED"  # Final approval by HR
    REJECTED = "REJECTED"  # Rejected by management or HR
    MANAGEMENT_REJECTED = "MANAGEMENT_REJECTED"  # Rejected by management
    COMPLETED = "COMPLETED"
    CANCELLED = "CANCELLED"
...
class Employee(db.Model):
    ...
    face_encoding = db.Column(LONGTEXT, nullable=True)  # Serialized face encodings (JSON array)
    ...
    def to_dict(self):
        ...
        # Ensure photo is a valid data URL for frontend display
        photo_value = self.photo
        if photo_value and not (photo_value.startswith('data:image/jpeg;base64,') or photo_value.startswith('data:image/png;base64,')):
            photo_value = f'data:image/jpeg;base64,{photo_value}'
        return {
            'id': self.id,
...
class Leave(db.Model):
    ...
    manager_approved_by = db.Column(db.Integer, db.ForeignKey('employees.id'))
    manager_approved_at = db.Column(db.DateTime)
    manager_notes = db.Column(db.Text)
    # HR/Final approval fields
    ...
    # Relationships
    manager_approver = db.relationship('Employee', foreign_keys=[manager_approved_by])
    ...
    def to_dict(self):
        ...
        return {
            ...
            'managerApprovedBy': self.manager_approved_by,
            'managerApproverName': self.manager_approver.full_name if self.manager_approver else None,
            'managerApprovedAt': self.manager_approved_at.isoformat() if self.manager_approved_at else None,
            'managerNotes': self.manager_notes,
            # HR/Final approval info
...
class TourIntimation(db.Model):
    """Tour/Business Trip Intimation model"""

    __tablename__ = 'tour_intimations'

    id = db.Column(db.Integer, primary_key=True)
    employee_id = db.Column(db.Integer, db.ForeignKey('employees.id'), nullable=False)
    employee_name = db.Column(db.String(200), nullable=False)
    tour_purpose = db.Column(db.String(500), nullable=False)
    destination = db.Column(db.String(200), nullable=False)
    start_date = db.Column(db.Date, nullable=False)
    end_date = db.Column(db.Date, nullable=False)
    duration_days = db.Column(db.Integer, nullable=False)
    estimated_cost = db.Column(db.Float)
    advance_required = db.Column(db.Float, default=0)
    travel_mode = db.Column(db.String(50))  # flight, train, bus, car, etc.
    accommodation_required = db.Column(db.Boolean, default=False)
    remarks = db.Column(db.Text)
    status = db.Column(db.Enum(TourStatus), default=TourStatus.PENDING)
    # Management approval fields
    management_approved_by = db.Column(db.Integer, db.ForeignKey('employees.id'))
    ...
    actual_cost = db.Column(db.Float)  # Filled after tour completion
    completion_remarks = db.Column(db.Text)  # Filled after tour completion

    created_at = db.Column(db.DateTime, default=get_ist_now)
    updated_at = db.Column(db.DateTime, default=get_ist_now, onupdate=get_ist_now)

    # Relationships
    employee = db.relationship('Employee', foreign_keys=[employee_id], backref='tours')
    management_approver = db.relationship('Employee', foreign_keys=[management_approved_by])
    approver = db.relationship('Employee', foreign_keys=[approved_by])

    def to_dict(self):
        """Convert tour intimation to dictionary"""
        # Handle status - could be enum or string
        if isinstance(self.status, TourStatus):
            status_value = self.status.value.lower()
        elif isinstance(self.status, str):
            status_value = self.status.lower()
        else:
        ...
        return {
            ...
            'tourPurpose': self.tour_purpose,
            'destination': self.destination,
            'startDate': self.start_date.isoformat(),
            'endDate': self.end_date.isoformat(),
            'durationDays': self.duration_days,
            'estimatedCost': self.estimated_cost,
            'advanceRequired': self.advance_required,
            'travelMode': self.travel_mode,
            'accommodationRequired': self.accommodation_required,
            'remarks': self.remarks,
            'status': status_value,
            # Management approval info
            'managementApprovedBy': self.management_approved_by,
            'managementApproverName': self.management_approver.full_name if self.management_approver else None,
            'managementApprovedAt': self.management_approved_at.isoformat() if self.management_approved_at else None,
            'managementNotes': self.management_notes,
            # HR/Final approval info
            'approvedBy': self.approved_by,
            'approverName': self.approver.full_name if self.approver else None,
            'approvedAt': self.approved_at.isoformat() if self.approved_at else None,
            ...
            'completionRemarks': self.completion_remarks,
            'createdAt': self.created_at.isoformat(),
            'updatedAt': self.updated_at.isoformat()
        }
