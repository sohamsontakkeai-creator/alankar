...
class GuestList(db.Model):
    ...
    status = db.Column(db.Enum('scheduled', 'checked_in', 'checked_out', 'cancelled'), default='scheduled', nullable=False)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'status': self.status if isinstance(self.status, str) else (self.status.value if isinstance(self.status, GuestStatus) else str(self.status)),
            'notes': self.notes,
