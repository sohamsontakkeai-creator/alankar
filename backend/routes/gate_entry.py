...
def recognize_face():
    ...
    try:
        ...
        # LBPH confidence is 0-100+, lower is better, so confidence = 100 - distance
        confidence_percent = max(0, 100 - result['distance'])
        
        if action == 'entry':
            entry_result = gate_entry_service_db.manual_entry(
                user.phone, 
                f"Face recognition entry (confidence: {confidence_percent:.1f}%)",
                override_cooling=False
            )
        else:
            entry_result = gate_entry_service_db.manual_exit(
                user.phone,
                f"Face recognition exit (confidence: {confidence_percent:.1f}%)",
                override_cooling=False
            )
