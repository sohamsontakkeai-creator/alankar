...
class HRService:
    ...
    def get_dashboard_data():
        ...
        try:
            ...
            recent_leaves = Leave.query.filter(
                Leave.status == LeaveStatus.APPROVED,
                Leave.approved_at >= get_ist_now() - timedelta(days=7)
    ...
    def approve_leave_request(leave_id, approved=True, approver_id=None):
        """Approve or reject a leave request (HR final approval)"""
        leave = Leave.query.get(leave_id)
        if not leave:
            raise ValueError('Leave request not found')
        
        # Check if leave has been manager-approved first
        if leave.status != LeaveStatus.MANAGER_APPROVED:
            raise ValueError('Leave request must be approved by manager first')
        ...
        if approver_id:
            ...
            leave.approved_at = get_ist_now()

        db.session.commit()
        return leave.to_dict()
    
    @staticmethod
    def approve_leave_as_manager(leave_id, manager_id, approved=True, notes=None):
        ...
        if approved:
            ...
            leave.manager_approved_by = manager_id
            leave.manager_approved_at = get_ist_now()
    ...
    def get_team_leave_requests(manager_id):
        ...
        team_employees = Employee.query.filter_by(manager_id=manager_id).all()
        employee_ids = [emp.id for emp in team_employees]
        
        if not employee_ids:
            return []
        
        # Get their leave requests (pending or manager-approved)
        leaves = Leave.query.filter(
            Leave.employee_id.in_(employee_ids),
            Leave.status.in_([LeaveStatus.PENDING, LeaveStatus.MANAGER_APPROVED])
    ...
    def get_management_pending_leaves():
        ...
        # Get all employees with HR or Manager designation
        hr_manager_employees = Employee.query.filter(
            (Employee.designation.ilike('%HR%')) | (Employee.designation.ilike('%Manager%'))
        ).all()
        
        employee_ids = [emp.id for emp in hr_manager_employees]
        ...
        for leave in leaves:
            try:
                ...
                # Add employee designation to the response
                employee = Employee.query.get(leave.employee_id)
                if employee:
                    leave_dict['employeeDesignation'] = employee.designation
    ...
    @staticmethod
    def approve_leave_by_management(leave_id, approved=True):
        """Management approves or rejects leave request from HR/Manager employees"""
        leave = Leave.query.get(leave_id)
        if not leave:
            raise ValueError('Leave request not found')
        
        # Verify this is from an HR or Manager employee
        employee = Employee.query.get(leave.employee_id)
        if not employee:
            raise ValueError('Employee not found')
        
        designation = employee.designation or ''
        if 'HR' not in designation and 'Manager' not in designation:
            raise ValueError('This leave request is not from an HR or Manager employee')
    ...
    @staticmethod
    def get_employees_on_leave(date_str=None):
        """Get list of employees currently on leave for a specific date"""
        try:
            ...
            # Get approved leaves that include the target date
            current_leaves = Leave.query.filter(
                ...
                try:
                    ...
                    if employee:
                        leave_info = {
                            ...
                            'phone': employee.phone,
                            'leaveType': leave.leave_type.value if leave.leave_type else 'N/A',
                        ...
                        employees_on_leave.append(leave_info)
                except Exception as e:
            ...
            print(f"Error fetching employees on leave: {e}")
            return {
    ...
    def update_interview_status(interview_id, status, feedback=None, rating=None, decision=None, interviewer_id=None):
        ...
        if interviewer_id:
            ...
            interview.conducted_at = get_ist_now()
    ...
    # Tour Intimation methods
    @staticmethod
    def get_tour_intimations(employee_id=None, status=None):
        """Get tour intimations with optional filters"""
        from models.hr import TourIntimation, TourStatus
        
        query = TourIntimation.query
        ...
        if status:
            try:
                status_enum = TourStatus[status.upper()]
                query = query.filter_by(status=status_enum)
            except KeyError:
                pass
        
        tours = query.order_by(TourIntimation.created_at.desc()).all()
    ...
    def get_tour_intimation(tour_id):
        ...
        tour = TourIntimation.query.get(tour_id)
        if not tour:
            raise ValueError(f"Tour intimation with ID {tour_id} not found")
        
        return tour.to_dict()
    
    @staticmethod
    def create_tour_intimation(data):
        ...
        tour = TourIntimation(
            ...
            tour_purpose=data['tourPurpose'],
            destination=data['destination'],
            start_date=start_date,
            end_date=end_date,
            duration_days=duration_days,
            estimated_cost=data.get('estimatedCost', 0),
            advance_required=data.get('advanceRequired', 0),
            travel_mode=data.get('travelMode', ''),
            accommodation_required=data.get('accommodationRequired', False),
    ...
    @staticmethod
    def update_tour_intimation(tour_id, data):
        ...
        # Update fields
        if 'tourPurpose' in data:
            tour.tour_purpose = data['tourPurpose']
        ...
        if 'estimatedCost' in data:
            tour.estimated_cost = data['estimatedCost']
        if 'advanceRequired' in data:
            tour.advance_required = data['advanceRequired']
        if 'travelMode' in data:
            tour.travel_mode = data['travelMode']
        if 'accommodationRequired' in data:
            tour.accommodation_required = data['accommodationRequired']
        if 'remarks' in data:
    ...
    @staticmethod
    def approve_tour_intimation(tour_id, approver_id):
        """HR approves a tour intimation (final approval after management)"""
        from models.hr import TourIntimation, TourStatus
        from datetime import datetime
        ...
        # HR can only approve tours that are management approved
        if tour.status != TourStatus.MANAGEMENT_APPROVED:
            raise ValueError(f"Tour must be approved by management first")
        
        tour.status = TourStatus.APPROVED
        tour.approved_by = approver_id
        tour.approved_at = get_ist_now()
        
        db.session.commit()
        return tour.to_dict()
    
    @staticmethod
    def reject_tour_intimation(tour_id, approver_id, reason):
        """HR rejects a tour intimation"""
        from models.hr import TourIntimation, TourStatus
        from datetime import datetime
        ...
        # HR can reject tours that are management approved
        if tour.status != TourStatus.MANAGEMENT_APPROVED:
            raise ValueError(f"Tour must be approved by management first before HR can reject")
    ...
    @staticmethod
    def complete_tour_intimation(tour_id, actual_cost, completion_remarks):
        ...
        tour.actual_cost = actual_cost
        tour.completion_remarks = completion_remarks
    ...
    def get_management_pending_tours():
        ...
        # Get tours with PENDING status (waiting for management approval)
        tours = TourIntimation.query.filter_by(status=TourStatus.PENDING).order_by(TourIntimation.created_at.desc()).all()
        return [tour.to_dict() for tour in tours]
    
    @staticmethod
    def approve_tour_by_management(tour_id, approved, notes=None):
        ...
        if tour.status != TourStatus.PENDING:
            raise ValueError(f"Tour intimation is not in pending status")
        
        if approved:
            # Management approved - now waiting for HR final approval
            tour.status = TourStatus.MANAGEMENT_APPROVED
            tour.management_approved_at = get_ist_now()
            tour.management_notes = notes
        else:
            # Management rejected
            tour.status = TourStatus.MANAGEMENT_REJECTED
            tour.management_approved_at = get_ist_now()
            tour.rejection_reason = notes or 'Rejected by management'
