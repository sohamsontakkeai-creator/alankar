...
class DispatchService:
    ...
    def get_all_dispatch_orders():
        ...
        try:
            ...
            for request in dispatch_requests:
                ...
                # Get company name and vehicle number from sales order or transport job
                company_name = '-'
                vehicle_number = None
                transport_job = None
                ...
                orders.append({
                    ...
                    'vehicleNumber': vehicle_number,  # Add vehicle number for company vehicles
                    'transporterName': company_name  # Also add as transporterName for consistency
                })
    ...
    def _process_self_delivery(dispatch_request, action_data):
        ...
        try:
            ...
            dispatch_request.updated_at = get_ist_now()
    ...
    def _process_company_delivery(dispatch_request, action_data):
        ...
        try:
            ...
            # Mark as ready for loading (appears in Company Vehicle Loading Queue immediately)
            dispatch_request.status = 'ready_for_load'
            ...
            if existing_job:
                ...
                existing_job.updated_at = get_ist_now()
            ...
            # Create transport job (no existing job) - status is 'pending' until transport assigns vehicle
            transport_job = TransportJob(
                dispatch_request_id=dispatch_request.id,
                transporter_name=action_data.get('transporterName'),
                vehicle_no=action_data.get('vehicleNo'),
                status='pending'  # Will change to 'sent_to_dispatch' when transport assigns vehicle
            )
    ...
    def verify_customer_pickup(gate_pass_id, verification_data):
        ...
        try:
            ...
            gate_pass.verified_at = get_ist_now()
    ...
    def get_dispatch_summary():
        ...
        try:
            ...
            # Count part load delivery loading queue using original delivery type (case-insensitive)
            part_load_loading_queue = DispatchRequest.query.filter(
                db.func.lower(DispatchRequest.original_delivery_type) == 'part load',
                DispatchRequest.status.in_(['ready_for_load', 'ready_for_pickup', 'entered_for_pickup'])
            ).count()
            
            # Count company vehicle loading queue (ready_for_load with company/free delivery types)
            company_vehicle_loading_queue = DispatchRequest.query.filter(
                db.func.lower(DispatchRequest.delivery_type).in_(['transport', 'company', 'free']),
                DispatchRequest.status == 'ready_for_load',
                db.func.lower(DispatchRequest.original_delivery_type).in_(['company delivery', 'free delivery', 'company', 'free'])
            ).count()
            ...
            return {
                ...
                'companyVehicleLoadingQueue': company_vehicle_loading_queue,
    ...
    @staticmethod
    def complete_company_vehicle_loading(dispatch_id, notes=None):
        """Mark loading completed for a company/free delivery order. Dispatch step before transport delivery."""
        try:
            dispatch_request = DispatchRequest.query.get(dispatch_id)
            if not dispatch_request:
                raise ValueError('Dispatch request not found')
            
            # Verify this is a company/free delivery order
            if dispatch_request.delivery_type != 'transport':
                raise ValueError('Loading completion is only valid for company/free delivery orders')
            
            # Only allow after vehicle has been assigned by transport
            if dispatch_request.status not in ['ready_for_load', 'assigned_transport']:
                raise ValueError(f'Order must be in ready_for_load status, currently: {dispatch_request.status}')
            
            # Get the transport job
            transport_job = TransportJob.query.filter_by(dispatch_request_id=dispatch_request.id).first()
            if not transport_job:
                raise ValueError('No transport job found for this dispatch request')
            
            # Update statuses
            dispatch_request.status = 'loaded'
            if notes:
                dispatch_request.dispatch_notes = (dispatch_request.dispatch_notes or '') + f" | Company Vehicle Loaded: {notes}"
            dispatch_request.updated_at = get_ist_now()
            
            # Update transport job status to in_transit (ready for delivery)
            transport_job.status = 'in_transit'
            transport_job.updated_at = get_ist_now()
            ...
            return {
                'status': 'loaded',
                'message': 'Company vehicle loading completed. Ready for delivery',
                'dispatchRequest': dispatch_request.to_dict(),
                'transportJob': transport_job.to_dict()
            }
        except Exception as e:
            db.session.rollback()
            raise Exception(f"Error completing company vehicle loading: {str(e)}")
