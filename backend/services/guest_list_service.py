        ...
        try:
            ...
            if filters:
                ...
                if filters.get('status'):
                    query = query.filter(GuestList.status == filters['status'])
    ...
    def get_guest_summary():
        ...
        try:
            ...
            todays_scheduled = GuestList.query.filter(
                and_(GuestList.visit_date == today, GuestList.status == 'scheduled')
            ).count()

            todays_checked_in = GuestList.query.filter(
                and_(GuestList.visit_date == today, GuestList.status == 'checked_in')
            ).count()

            todays_checked_out = GuestList.query.filter(
                and_(GuestList.visit_date == today, GuestList.status == 'checked_out')
            ).count()

            todays_cancelled = GuestList.query.filter(
                and_(GuestList.visit_date == today, GuestList.status == 'cancelled')
            ).count()

            # Total stats
            total_scheduled = GuestList.query.filter(GuestList.status == 'scheduled').count()
            total_checked_in = GuestList.query.filter(GuestList.status == 'checked_in').count()
            total_checked_out = GuestList.query.filter(GuestList.status == 'checked_out').count()
