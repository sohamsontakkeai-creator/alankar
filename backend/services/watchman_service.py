...
class WatchmanService:
    ...
    def verify_customer_identity(gate_pass_id, verification_data, action='release'):
        ...
        print(f"========== WATCHMAN SERVICE CALLED - Gate Pass ID: {gate_pass_id}, Action: {action} ==========")
        try:
            gate_pass = GatePass.query.get(gate_pass_id)
            print(f"[SERVICE] Gate pass found: {gate_pass is not None}, Party: {gate_pass.party_name if gate_pass else 'N/A'}")
            if not gate_pass:
            ...
            if action == 'send_in':
                ...
                dispatch_request.dispatch_notes = f"{dispatch_request.dispatch_notes or ''} | Watchman verified and sent vehicle in for loading at {get_ist_now().strftime('%Y-%m-%d %H:%M:%S')}"
                dispatch_request.updated_at = get_ist_now()

                db.session.commit()

                # Create audit log (will be overridden by route layer if user info is available)
                try:
                    from services.audit_service import AuditService
                    from models import AuditAction, AuditModule
                    
                    # Get order number from dispatch request -> sales order
                    order_number = 'N/A'
                    if dispatch_request and dispatch_request.sales_order_id:
                        sales_order = SalesOrder.query.get(dispatch_request.sales_order_id)
                        if sales_order:
                            order_number = sales_order.order_number or 'N/A'
                    
                    # Try to get user from verification_data, fallback to generic
                    user_name = verification_data.get('verifiedBy', 'Security User')
                    description = f"{user_name} sent customer in for loading - Customer: {gate_pass.party_name}, Order: {order_number}, Vehicle: {gate_pass.vehicle_no or 'N/A'}"
                    ...
                    AuditService.log_activity(
                        ...
                        new_values={
                            'action': 'send_in',
                            'customer_name': gate_pass.party_name,
                            'order_number': order_number,
                            'vehicle': gate_pass.vehicle_no,
                ...
                try:
                    ...
                    # Try to get user from verification_data, fallback to generic
                    user_name = verification_data.get('verifiedBy', 'Security User')
                    description = f"{user_name} released customer after loading - Customer: {gate_pass.party_name}, Order: {order_number}, Vehicle: {gate_pass.vehicle_no or 'N/A'}"
    ...
    def reject_pickup(gate_pass_id, rejection_reason):
        ...
        try:
            ...
            gate_pass.verified_at = get_ist_now()  # Use verified_at to track when it was processed
            ...
            try:
                ...
                description = f"Security User rejected pickup - Customer: {gate_pass.party_name}, Order: {order_number}, Reason: {rejection_reason}"
