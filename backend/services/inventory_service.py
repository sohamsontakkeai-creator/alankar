...
class InventoryService:
    ...
    def process_purchase_verification(purchase_order_id):
        """Verify purchase and allocate materials directly to production, adding only excess to inventory"""
        try:
            order = PurchaseOrder.query.get_or_404(purchase_order_id)
            
            print(f"\n[STORE VERIFICATION] Processing PO #{purchase_order_id}")
            print(f"[STORE VERIFICATION] Current status: {order.status}")
            ...
            # Get purchased shortage materials (what was bought to cover shortage)
            purchased_materials = order.get_materials_list()
            print(f"[STORE VERIFICATION] Purchased shortage materials: {purchased_materials}")
            
            # Get original requirements (full original quantities needed for production)
            original_requirements = order.get_original_requirements()
            print(f"[STORE VERIFICATION] Original requirements: {original_requirements}")
            
            # Get extra materials (additional items purchased)
            extra_materials = order.get_extra_materials()
            print(f"[STORE VERIFICATION] Extra materials: {extra_materials}")
            
            if not original_requirements:
                # If no original requirements, this is a direct purchase - add all to inventory
                print("[STORE VERIFICATION] No original requirements - adding all to general inventory")
                all_materials = purchased_materials + extra_materials
                for material in all_materials:
                    ...
                    if inventory_item:
                        ...
                        print(f"[STORE VERIFICATION]   {material_name}: Added {material_qty} to inventory")
                    else:
                        ...
                        print(f"[STORE VERIFICATION]   {material_name}: Created new item with {material_qty}")
            else:
                # Process purchased shortage materials - allocate directly to production
                print("[STORE VERIFICATION] Allocating purchased shortage materials directly to production...")
                ...
                for material in purchased_materials:
                    ...
                    material_name = material['name']
                    purchased_qty = material.get('quantity', 0)  # This is the shortage quantity
                    
                    print(f"[STORE VERIFICATION] Processing {material_name}:")
                    print(f"[STORE VERIFICATION]   Shortage purchased: {purchased_qty}")
                    ...
                    if not inventory_item:
                        ...
                        db.session.add(inventory_item)
                        db.session.flush()
                        print(f"[STORE VERIFICATION]   Created new inventory item")
                    
                    # All purchased shortage materials are allocated directly to production (reserved)
                    # They don't go to general inventory
                    print(f"[STORE VERIFICATION]   Allocated {purchased_qty} directly to production (reserved for original requisition)")
                
                # Process extra materials - all go to general inventory
                print("[STORE VERIFICATION] Processing extra materials...")
                for material in extra_materials:
                    ...
                    if inventory_item:
                        inventory_item.add_stock(material_qty)
                        print(f"[STORE VERIFICATION]   {material_name}: Added {material_qty} extra to inventory")
                    else:
                        inventory_item = StoreInventory(
                            name=material_name,
                            quantity=material_qty,
                        ...
                        db.session.add(inventory_item)
                        print(f"[STORE VERIFICATION]   {material_name}: Created new item with {material_qty} extra")
            ...
            if production_order:
                production_order.status = 'materials_allocated'
                print(f"[STORE VERIFICATION] Production order #{production_order.id} status updated to materials_allocated")
            
            db.session.commit()
            
            return {
                'message': 'Purchase verified. Shortage materials allocated to production, extra materials added to inventory.',
                'allAvailable': True,
                'newStatus': order.status,
                'purchaseOrder': order.to_dict()
            }
