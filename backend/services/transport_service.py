...
class TransportService:
        ...
        try:
            ...
            for request in all_dispatches.values():
                ...
                order_data = {
                    ...
                    # Add transport cost from sales order (always get latest)
    ...
    def update_part_load_delivery_details(sales_order_id, delivery_data):
        ...
        try:
            ...
            part_load_detail.updated_at = get_ist_now()
        ...
        try:
            ...
            for request in approval_requests:
                ...
                if sales_order:
                    ...
                    # Add transport cost mapping for frontend compatibility - always use current cost from sales order
                    approval_data['transportCost'] = sales_order.transport_cost or 0.0
            ...
            for job in transport_jobs:
                ...
                jobs.append({
                    ...
                    'priority': 'high' if dispatch_request.created_at and \
                               (get_ist_now() - dispatch_request.created_at).days > 1 else 'normal'
                })
        ...
        try:
            ...
            if vehicle_no:
                ...
                if fleet_vehicle:
                    ...
                    fleet_vehicle.updated_at = get_ist_now()
            ...
            # Update transport job - set to 'sent_to_dispatch' so it appears in Company Vehicle Loading Queue
            transport_job.transporter_name = transporter_name
            transport_job.vehicle_no = vehicle_no
            transport_job.status = 'sent_to_dispatch'  # Changed from 'assigned' to indicate vehicle assigned but not loaded yet
            transport_job.updated_at = get_ist_now()
            
            # Update dispatch request status - set to 'ready_for_load' so dispatch can load it
            dispatch_request = DispatchRequest.query.get(transport_job.dispatch_request_id)
            if dispatch_request:
                dispatch_request.status = 'ready_for_load'  # Changed from 'assigned_transport' to indicate ready for loading
                dispatch_request.dispatch_notes = f"Vehicle assigned by transport: {transport_job.transporter_name}"
                if transport_job.vehicle_no:
                    dispatch_request.dispatch_notes += f" (Vehicle: {transport_job.vehicle_no})"
                dispatch_request.dispatch_notes += " - Ready for loading"
                dispatch_request.updated_at = get_ist_now()
    ...
    def get_in_transit_deliveries():
        ...
        try:
            ...
            for job in transport_jobs:
                ...
                days_in_transit = (get_ist_now() - job.updated_at).days
    ...
    def get_transport_summary():
        ...
        try:
            ...
            three_days_ago = get_ist_now() - timedelta(days=3)
    ...
    def update_fleet_vehicle(vehicle_id, vehicle_data):
        ...
        try:
            ...
            vehicle.updated_at = get_ist_now()
            db.session.commit()
